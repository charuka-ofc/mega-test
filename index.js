...
async function downloadAndExtractZip() {
  try {
    ...
    console.log('MIZUKI MD ZIP file downloaded successfully ✅');
...
if (!fs.existsSync(__dirname + `/${config.SESSION_NAME}/creds.json`)) {
    if (!config.SESSION_ID) return console.log("Please Add SESSION_ID 🚫")
      const sessdata = config.SESSION_ID.replace("MIZUKI-MD=", "")
...
//Run the function
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        //============
            ...
            //===============
            console.log('Plugins installed Successfull ✅')
	    //await connectdb()
            //await updb()
            await start_numrep_process();
            //========================
            console.log('MIZUKI MD Whatsap Bot Conected ✅')
        joinGroupFromJson();        
const prefix = config.PREFIX

let up = `✅ 𝙼𝙸𝚉𝚄𝙺𝙸 𝙼𝙳 𝙲𝙾𝙽𝙴𝙲𝚃𝙴𝙳 𝚂𝚄𝙲𝙲𝙴𝚂𝚂 ✅ 

*𝙵𝙾𝙻𝙻𝙾𝚆 𝙼𝙸𝚉𝚄𝙺𝙸 𝙲𝙷𝙰𝙽𝙴𝙻:* https://whatsapp.com/channel/0029Vau5Xo2L2ATxRfwiHX2e
*ʀᴇᴘᴏ ʟɪɴᴋ*: https://github.com/charuka-ofc/MIZUKI--MD
*ꜱᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ*: https://chat.whatsapp.com/F5XfyMJMWdE7Qd74BnE5Fz

ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴍɪᴢᴜᴋɪ ᴍᴅ ᴏᴡɴᴇʀ`;

conn.sendMessage(config.OWNER_NUMBER + "@s.whatsapp.net", { image: { url: `https://i.ibb.co/h1yqNpJS/IMG-20250501-084204.png` }, caption: up })



const { startNewsService } = require('./plugins/main'); 
   
const newsjid = config.NEWS_JID;
if (config.AUTO_NEWS_SENDER === "true"){
startNewsService(conn,newsjid);
}
...
async function joinGroupFromJson() {
    try {
        let joinlink2 = await fetchJson('https://raw.githubusercontent.com/charuka-ofc/MIZUKI-MD-DATABASE/refs/heads/main/BOT%20FILE');
...
const metadata = await conn.newsletterMetadata("jid", "120363376466846343@newsletter")	      
if (metadata.viewer_metadata === null){
await conn.newsletterFollow("120363376466846343@newsletter")
console.log("MIZUKI MD CHANNEL FOLLOW ✅")
}	 


const id = mek.key.server_id
await conn.newsletterReactMessage("120363376466846343@newsletter", id, "❤️‍🔥")
...
q = args.join(' ')		
  ...
  return await conn.sendMessage(from, { text: teks }, { quoted: mek })
      ...
      if (config.CHAT_BOT === 'true' && m.body) {
        let query = m.body.toLowerCase();
        try {
          let aiResponse = await fetchJson('https://saviya-kolla-api.koyeb.app/ai/saviya-ai?query=' + query);
          await conn.sendMessage(from, { text: aiResponse.result.data });
        } catch (err) {
    ...
    //await conn.sendMessage(from, { audio: fs.readFileSync("./" + type.ext), mimetype:  originalMessage.message.audioMessage.mimetype, fileName:  `${m.id}.mp3` })	
     const sdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'MIZUKI MD 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: sdata});
    
    }else{
    
    const stdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'MIZUKI MD 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: stdata});
...
conn.replyList2 = async (from , list_reply , options) => {
...
//================================band user====================================
const banbn = await fetchJson(`https://raw.githubusercontent.com/charuka-ofc/MIZUKI-MD-DATABASE/refs/heads/main/Bot_Ditels/Banduser.json`)
const plynYnna = banbn.split(",")
const isBanUser = [ ...plynYnna ]
      .map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net")
      .includes(sender)
const bandgroup = await fetchJson(`https://raw.githubusercontent.com/charuka-ofc/MIZUKI-MD-DATABASE/refs/heads/main/Bot_Ditels/Bandgroup.json`);
...
const ownerdata = (await axios.get('https://raw.githubusercontent.com/charuka-ofc/MIZUKI-MD-DATABASE/refs/heads/main/CONTROL/ditels.json')).data

           
                       config.FOOTER = ownerdata.footer
	           config.LOGO = ownerdata.logo
	           config.LOGO2 = ownerdata.logo2
	           config.DATABASE_URL = "postgresql://postgres:@Asitha2005b@db.waiqbrnuxkjebghzhovz.supabase.co:5432/postgres"
	           config.MENU = "menu"
	           config.CHANEL_LINK = ownerdata.chanel_link
...
const bad = await fetchJson(`https://raw.githubusercontent.com/charuka-ofc/MIZUKI-MD-DATABASE/refs/heads/main/Bot_Ditels/bad_word.json`)
...
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` 📚 *Removed By MIZUKI MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
    if ( mek.id.startsWith("QUEENAMDI") ) {
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` *💃 MIZUKI MD* ❗\n*Removed By MIZUKI MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
...
app.get("/", (req, res) => {
res.send("📟 MIZUKI-MD Working successfully!");
});
